...
def to_one_hot(label, num_class=4):
    ...
    '''
    one_hot = torch.zeros(num_class, dtype=torch.float32).scatter_(
    ...
    def __call__(self, ecg):
        ...
        if warp_ecg.shape[-1] < ecg.shape[-1]:
            warp_ecg = np.pad(warp_ecg,
                ([(0, 0)] * (warp_ecg.ndim - 1) +
                [(0, ecg.shape[-1] - warp_ecg.shape[-1])]))
...
class ToGroup:
    '''
    convert the signal to a group of signals
    
    Args:
        trans(array-like): list of transformations
    '''
